...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (
    req.files &&
    req.files.coverImage instanceof Array &&
  ...
  if (!createdUser) {
    throw new ApiError(
      508,
      "Something sad and went wrong while registering the user"
    );
  }
...
const refreshaccessToken = asyncHandler(async (req, res) => {
  try {
    ...
    throw new ApiError(405, error?.message || "Invalid Refresh Token catched");
  }
});
...
const ChangeUserPassword = asyncHandler(async (req, res) => {
  try {
    ...
    if (!isPasswordCorrect) {
      res.json(new ApiError(407, "Password is Not Correct ❌"));
    }
    ...
    throw new ApiError(
      405,
      "Failed while changing the password",
      error.message
    );
  }
});
...
const UpdateAccountDetails = asyncHandler(async (req, res) => {
  ...
  if (!(fullname || email)) {
    throw new ApiError(409, "All feild are Not Empty ");
  }
...
const updatecoverImage = asyncHandler(async (req, res) => {
  try {
    ...
    if (!coverImagepath) {
      res.json(new ApiError(419, "CoverImage Path is Missing "));
    }

    const CoverImageInstance = await uploadOnCloudinary(coverImagepath);

    if (!CoverImageInstance.url) {
      res.json(new ApiError(409, "CoverImage Not uploaded Successfully "));
    }
    ...
    throw new ApiError(406, "Error while Updating Cover Image ❌", error);
  }
});
...
const updateAvatarImage = asyncHandler(async (req, res) => {
  try {
    ...
    if (!avatarpath) {
      res.json(new ApiError(419, "Avatar Path is Missing "));
    }

    const AvatarInstance = await uploadOnCloudinary(avatarpath);

    if (!AvatarInstance.url) {
      res.json(new ApiError(409, "Avatar  Not uploaded"));
    }
...
const getUserProfile = asyncHandler(async (req, res) => {
  ...
  if (!username.trim()) {
    res.json(new ApiError(408, "User Name is Required"));
  }
  ...
  if (!channel?.length) {
    res.json(new ApiError(404, "Channel does not exists ❌"));
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        203,
        channel[0],
        "User Channel are fetched successfully ✅"
      )
    );
});
